...
from Backend.Schedulers.macro_step import *
...
class ActuatorScheduler(QObject):
    ...
    __sequence_execution_end = True
    def __init__(self):
        super().__init__()
        self.pid_controllers = [PIDController(0, 0, 0, 0, 200, 10),
                                PIDController(0, 0, 0, 0, 200, 10)]
        self.__vertical_controller_mode = [MacroStep.ActionMoveVertical.Mode.POSITION, MacroStep.ActionMoveVertical.Mode.POSITION]
        self.__vertical_target_forces = [0, 0]
        self.__linear_stage_velocity = 0
        self.__loop_timer = QTimer()
        self.__loop_timer.timeout.connect(self.run)
        self.__loop_timer.start(self.__sampling_time)
        ...
        if PhidgetInterface.get_connected() and HorizontalStageInterface.get_connected():
            ...
            match self.__step_execute_state:
                    ...
                    for actions in self.__step_loaded.actions:
                        if isinstance(actions, MacroStep.ActionMoveHorizontal):
                            ...
                            if actions.axis == MacroStep.ActionMoveVertical.Axis.ALL:
                                self.__vertical_controller_mode = [actions.mode, actions.mode]
                                if actions.mode == MacroStep.ActionMoveVertical.Mode.FORCE:
                                    self.__vertical_target_forces = [actions.target, actions.target]
                                    ...
                                    self.__vertical_target_positions = [actions.target, actions.target]
                            else:
                                self.__vertical_controller_mode[cast(int, actions.axis.value)] = actions.mode
                                if actions.mode == MacroStep.ActionMoveVertical.Mode.FORCE:
                                    self.__vertical_target_forces[cast(int, actions.axis.value)] = actions.target
                                elif actions.mode == MacroStep.ActionMoveVertical.Mode.POSITION:
                                    self.__vertical_target_positions[cast(int, actions.axis.value)] = actions.target
                            ...
                            if actions.axis == MacroStep.ActionChangeVerticalPIDParams.Axis.ALL:
                                ...
                                self.pid_controllers[cast(int, actions.axis.value)].set_pid_params(actions.kp, actions.ki, actions.kd,
                                                                       actions.i_limit, actions.out_limit)
                                self.pid_controllers[cast(int, actions.axis.value)].clear_errors()
                            ...
                            if actions.axis == MacroStep.ActionStopPrevious.Axis.ALL:
                                self.__vertical_target_forces = [0, 0]
                                self.__vertical_target_positions = [JRKInterface.get_position(VerticalAxis.AXIS_0), JRKInterface.get_position(VerticalAxis.AXIS_1)]
                                ...
                                self.__vertical_target_forces[cast(int, actions.axis.value)] = 0
                                self.__vertical_target_positions[cast(int, actions.axis.value)] = JRKInterface.get_position(VerticalAxis(cast(int, actions.axis.value)))
                            pass
                    self.__step_start_time = QDateTime.currentMSecsSinceEpoch()
                    self.__step_execute_state = ActuatorsControllerState.EXECUTING
                ...
                case ActuatorsControllerState.EXECUTING:
                    ...
                    for condition in self.__step_loaded.end_conditions:
                        ...
                        if isinstance(condition, MacroStep.EndConditionForce):
                            ...
                            condition_validation = QDateTime.currentMSecsSinceEpoch() - self.__step_start_time >= condition.wait_time
                    ...
                    if step_end_validation:
                        self.__step_execute_state = ActuatorsControllerState.END
                case ActuatorsControllerState.END:
                    if self.__sequence_index + 1 < len(self.__step_sequence):
                        self.__sequence_index += 1
                        print("End Step")
                        self.__step_loaded = self.__step_sequence[self.__sequence_index]
                        self.__step_execute_state = ActuatorsControllerState.START
                    else:
                        if not self.__sequence_execution_end:
                            self.__sequence_execution_end = True
                            print("Execution End")


            # Execute
            for i in range(VerticalAxis.AXIS_COUNT.value):
                match self.__vertical_controller_mode[i]:
                    case MacroStep.ActionMoveVertical.Mode.FORCE:
                        self.__vertical_target_forces[i] = -self.pid_controllers[i].update(
                            PhidgetInterface.get_calibrated_forces(VerticalAxis(i)),
                            self.__vertical_target_forces[i])
                        JRKInterface.set_duty_cycle(int(self.__vertical_target_forces[i]), VerticalAxis(i))
                    case MacroStep.ActionMoveVertical.Mode.POSITION:
                        JRKInterface.set_target_position(int(self.__vertical_target_positions[i]), VerticalAxis(i))

    def run_step_sequence(self, macro_steps: list[MacroStep]):
        if len(macro_steps) > 0:
            self.__step_sequence            = macro_steps
            self.__step_loaded              = macro_steps[0]
            self.__sequence_index           = 0
            self.__step_execute_state       = ActuatorsControllerState.START
            self.__sequence_execution_end   = False
    ...
    def stop(self):
        ...
        self.pid_controllers[0].clear_errors()
        self.pid_controllers[1].clear_errors()
        self.__vertical_target_positions = [JRKInterface.get_position(VerticalAxis.AXIS_0),
                                            JRKInterface.get_position(VerticalAxis.AXIS_1)]
        HorizontalStageInterface.send_target_position(HorizontalStageInterface.get_position())

    def get_execute_state(self):
        return self.__step_execute_state

    def get_sequence_execution_index(self):
        return self.__sequence_index

    def get_sequence_execution_end(self):
        return self.__sequence_execution_end