...
class ActionExecuteScheduler:
    ...
    __thread : QThread = QThread()
    __count : int = 0
    @staticmethod
    def init():
        # ActionExecuteScheduler.__loop_timer.moveToThread(ActionExecuteScheduler.__thread)
        ...
        try:
            if PhidgetInterface.get_connected() and HorizontalStageInterface.get_connected():
                if not ActionExecuteScheduler.initialized:
                    ...
                    if  ActionExecuteScheduler.__vertical_target_positions[0]!=0 and \
                        ActionExecuteScheduler.__vertical_target_positions[1]!=0:
